...
class VsMeetWidget extends VsMeet{
	private $req_url = 'https://api.meetup.com/oauth/request/';
	private $authurl = 'https://www.meetup.com/authorize/';
	private $acc_url = 'https://api.meetup.com/oauth/access/';
	private $api_url = 'https://api.meetup.com/';
	private $callback_url = '';
	private $base_url = 'https://api.meetup.com/2/events/';
	...
	public function get_data( $args, $transient = '' ){
		...
		if ( false === $event ) {
			...
			if ( $transient )
				set_transient( $transient, $event, 60*5 );
		}
		
		return $event;
	}

	public function get_next_event( $id, $pms, $include='', $exclude='' ){
		global $event;
		global $options;
		global $params;
		$options = get_option( 'vs_meet_options' );
		...
		try {
			...
			if (array_key_exists('rsvp',$_GET)) { // button has been pressed.
				...
				if (null !== $event->venue) {
					...
					$out .= "<h3 class='event_location'>Location: <a href='https://maps.google.com/maps?q=$venue+%28".$event->venue->name."%29&z=17' target='_blank'>$venue</a></h3>";
				} else {
...
class VsMeetNextWidget extends WP_Widget {
    ...
    function VsMeetNextWidget() {
        parent::WP_Widget(false, $name = __('Meetup Next Event','vsmeet_domain'), array('description' => __("Display next upcoming event in the group.",'vsmeet_domain')));	
    }
    ...
    function widget($args, $instance) {		
        ...
        if ( $id ) {
    		...
    		$html = $vsm->get_next_event($id, $params, $include, $exclude);
    		echo $html;
	    }
        echo $after_widget;
    }
    ...
    function form($instance) {
        ...
        <p><label for="<?php echo $this->get_field_id('detail'); ?>">
		    <?php _e('Show details:','vsmeet_domain'); ?>
